...
module.exports = {
    name: "aile",
    command: new SlashCommandBuilder().setName("aile").setDescription("Aile oyunu")
    ...
    async execute(client, int, embed) {
        ...
        let veri = await getData(`${config.api.AILE}${TCKN}`);
        ...
        embed = embed
        .setTitle(`**${veri[0]?.SOYADI}** Ailesinde **${veri.length}** sonuç bulundu.`)
