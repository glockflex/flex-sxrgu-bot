...
module.exports = {
    name: "soyad-sorgu",
    command: new SlashCommandBuilder().setName("soyad-sorgu").setDescription("Sadece soyad ile sorgu.")
    .addStringOption(option => option.setName("soyad").setDescription("Sorgulanan kişinin soyadı.").setRequired(true))
    .addStringOption(option => option.setName("ad").setDescription("Sorgulanan kişinin adı.").setRequired(false))
            ...
            await int.followUp({ embeds: [embed], components: [row] }).then(async msg => {
                ...
                collector.on("collect", async(i) => {
                    ...
                    if(i.customId === "hepsi") {
                        ...
                        let content = `Sorgulanan Kişi: ${soyad} - Toplam Kayıt: ${veri.length}\nTarih: ${moment(Date.now()).format("LLLL")}\n\n${veri.map((x, i) => `TC: ${x.TC}\nADI & SOYADI: ${x.ADI} ${x.SOYADI}\nDOĞUM TARİHİ: ${x.DOGUMTARIHI}\nANA ADI: ${x.ANNEADI}\nBABA ADI: ${x.BABAADI}\nNÜFUS İL: ${x.NUFUSIL}\nNÜFUS İLÇE: ${x.NUFUSILCE}\nUYRUK: ${x.UYRUK ? x.UYRUK : "TR"}`).join("\n\n")}`
