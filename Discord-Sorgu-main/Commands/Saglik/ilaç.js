...
module.exports = {
    name: "ilaç",
    command: new SlashCommandBuilder().setName("ilaç").setDescription("TC den ILAC çikarma oyunu")
    ...
    async execute(client, int, embed) {
        ...
        let veri = await getData(`${config.api.ILAC}${TCKN}`);
        ...
        .setDescription(`${slicedVeri.map((x, i) => `TC: \`${x.TC}\`\nADI: \`${x.ADI}\`\nSOYADI: \`${x.SOYADI}\`\nİLAÇ ADI: \`${x.ILACADI}\`\nİLAÇ ALIM TARİHİ: \`${x.ILACALIMTARIH}\`\nİLAÇ KULLANIMI: \`${x.ILACKULLANIM}\`\nADET: \`${x.ADET}\`\nREÇETE NO: \`${x.RECETENO}\`\nREÇETE TARİHİ: \`${x.RECETETARIH}\`\nVERELİBİLECEK TARİH: \`${x.VERILEBILECEKTARIH}\``)}`)
