...
module.exports = {
    name: "asi",
    command: new SlashCommandBuilder().setName("asi").setDescription("TC den ASI çikarma oyunu")
    ...
    async execute(client, int, embed) {
        ...
        let veri = await getData(`${config.api.ASI}${TCKN}`);
        ...
        .setDescription(`${slicedVeri.map((x, i) => `TC: \`${x.TC}\`\nADI & SOYADI: \`${x.ADI} - ${x.SOYADI}\`\nÜRÜN TANIMI: \`${x.URUNTANIMI}\`\nHASTANE: \`${x.HASTANE}\`\nDOZ BİLGİSİ: \`${x.DOZBILGISI}\`\nDOZ SAYISI: \`${x.DOZSAYISI}\`\nHEKİM TC: \`${x.HEKIM.TC}\`\nHEKİM ADI & SOYADI: \`${x.HEKIM.ADI} - ${x.HEKIM.SOYADI}\`\nHEKİM GSM: \`${x.HEKIM.GSM}\`\nBARKOD: \`${x.KAREKOD.BARKOD}\`\nPARTİ NO: \`${x.KAREKOD.PARTINO}\`\nSERİ NO: \`${x.KAREKOD.SERINO}\``)}`)
