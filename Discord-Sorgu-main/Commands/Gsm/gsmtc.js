...
module.exports = {
    name: "gsm-to-tc",
    command: new SlashCommandBuilder().setName("gsm-to-tc").setDescription("GSM den TC çikarma oyunu")
    .addNumberOption(option => option.setName("gsm").setDescription("Sorgunalacak GSM.").setRequired(true)),
    ownerOnly: false,
    async execute(client, int, embed) {

        let GSM = int.options.getNumber("gsm");

        let veri = await getData(`${config.api.GSMTC}${GSM}`);

        if (veri.success === false) return await int.followUp({ embeds: [embed.setDescription("Böyle bir GSM bulunamadı.")], ephemeral: true });
        ...
        await int.followUp({ embeds: [embed], components: [row] }).then(async (msg) => {
            ...
            collector.on("collect", async(i) => {
                ...
                if(i.customId === "sayfa") {
                    ...
                    let content = `Sorgulanan GSM: ${GSM} - Toplam Kayıt: ${slicedVeri.length}\nTarih: ${moment(Date.now()).format("LLLL")}\n\n${slicedVeri.map(x => `TC: ${x.TC}\nADI: ${x.ADI}\nSOYADI: ${x.SOYADI}\nGSM: ${x.GSM}`).join("\n\n")}`
                ...
                if(i.customId === "hepsi") {
                    ...
                    let content = `Sorgulanan GSM: ${GSM} - Toplam Kayıt: ${veri.length}\nTarih: ${moment(Date.now()).format("LLLL")}\n\n${veri.map(x => `TC: ${x.TC}\nADI: ${x.ADI}\nSOYADI: ${x.SOYADI}\nGSM: ${x.GSM}`).join("\n\n")}`
